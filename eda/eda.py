...
# RDKit helpers
# -------------------------
def _mols_from_smiles(smiles: pd.Series, max_mols: Optional[int], seed: int) -> Tuple[List[Any], Dict[str, Any]]:
...
def _compare_chemical_space(
    ...
    molA, statA = _mols_from_smiles(df_a[smiles_col_a], max_mols=max_mols, seed=seed)
    molB, statB = _mols_from_smiles(df_b[smiles_col_b], max_mols=max_mols, seed=seed + 1)
    ...
    if do_tsne and SKLEARN_AVAILABLE:
        try:
            ...
            if outdir is not None:
                outdir.mkdir(parents=True, exist_ok=True)
                df_coords = pd.DataFrame({"tsne1": Z[:, 0], "tsne2": Z[:, 1], "dataset": np.where(y_sub == 0, "A", "B")})
    ...
    if do_umap:
        if not UMAP_AVAILABLE:
            ...
            try:
                ...
                if outdir is not None:
                    outdir.mkdir(parents=True, exist_ok=True)
                    df_coords = pd.DataFrame({"umap1": Z[:, 0], "umap2": Z[:, 1], "dataset": np.where(y_sub == 0, "A", "B")})
...
def analyze_csv(input_file: str, output_file: str, cfg: EDAConfig, compare_to: Optional[str]) -> None:
    ...
    if RDKIT_AVAILABLE and smiles_col is not None and smiles_col in df.columns:
        mols, smi_stats = _mols_from_smiles(df[smiles_col], max_mols=cfg.max_rdkit_mols, seed=cfg.seed)
    ...
    if compare_to:
        try:
            df2 = pd.read_csv(compare_to, dtype=str, low_memory=False)
            if cfg.empty_as_missing:
                df2 = _empty_to_nan(df2)

            smiles_col_b = smiles_col if (smiles_col and smiles_col in df2.columns) else (df2.columns[0] if len(df2.columns) else "")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--compare-to", default=None, help="Optional second CSV to compare chemical space against")
